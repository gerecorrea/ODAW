  ...
  <nav class="navmenu">
    ...
    <article class="articlePagina3">
      <?php
        ...
        $query = "SELECT * FROM serie ORDER BY titulo";
        $result = mysqli_query($link, $query);

        echo "<h1><center> Listagem de séries <center></h1><br><br>";

        // echo "<br><br><a href='ExtraPages/SeriesMaisFavoritadas.php' class='card-foot botao' style='color: #000'>Séries mais favoritadas</a>";
        // echo "<a href='ExtraPages/SeriesOrdemLancamento.php' class='card-foot botao' style='color: #000'>Séries mais recentes</a><br><br>";
        //echo "<br><br><br><br><br><br><br>";

        while ($row = mysqli_fetch_row($result)) {
          echo "<div class='card'> <img src='".$row[14]. "'/>"; //imagem do filme.
          echo "<div class='card-info'>";
          echo "<h4>".$row[1]."</h4>"; //título
          echo "<div class='boxFav>";
          echo "<h6 class='botao' style='color: #000'>Favoritar</h6>";
          echo "</div>";
          echo "<h6>".$row[6]."&nbsp&nbsp".$row[7]." min&nbsp&nbsp".$row[8]." temporadas&nbsp&nbsp".$row[8]*$row[9]." episódios</h6>"; //ano, duração e gênereco.
          echo "<p>".$row[5]."</p>"; //sinopse
          echo "<br><h6> Direção: ".$row[2]; //direção
          echo "<h6> Escritores: ".$row[13]; //escritores
          echo "<h6> Elenco: ".$row[3]; //elenco
          echo "<h6> Personagens: ".$row[4]; //personagens
          echo "<h6> País: ".$row[11]; //país
          echo "<h6> Idioma: ".$row[12]."<br>"; //idioma
          echo "<h6> Fãs: ".$row[17]; //favoritos

          echo "<br><a href=\"favoritarSerie.php?id=".$row[0]."\" class='card-foot botao' style='color: #000'>Favoritar</a></tr>";
          ...
          echo "<a href=\"alterarSerie.php?id=".$row[0]."\" class='card-foot botao' style='color: #000'>Alterar</a></tr>";
          echo "<a href=\"deletarSerie.php?id=".$row[0]."\" class='card-foot botao' style='color: #000' >Deletar</a> &nbsp";

          echo "</div><div class='card-baixodir'>";
          echo "<a href='".$row[15]."' class='card-foot'>IMDb</a> &nbsp"; //gravar no bd link do imdb da pg
          echo "<a href='".$row[16]."' class='card-foot'>Assista agora!</a> </div> </div>"; //gravar no bd link do coisa no netflix, amazon prime, etc
        }
